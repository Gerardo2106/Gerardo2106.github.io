...
zipCodeInput.addEventListener("input", async function () {
    ...
    try {
        ...
        if (data.city) {
            ...
            document.querySelector("#cityDisplay").textContent = "Zip code not found.";
            document.querySelector("#latDisplay").textContent = "";
            document.querySelector("#lonDisplay").textContent = "";
        }
    } catch (err) {
        if (err instanceof TypeError) {
            alert("Error accessing API endpoint (network failure)");
